...
class RAGMetrics:
    ...
    def calculate_ndcg_at_k(
        self, retrieved_docs: List[str], relevance_scores: List[float], k: int
    ) -> float:
        ...
        dcg = sum(
            score / np.log2(i + 2) for i, score in enumerate(relevance_scores[:k])
        )
    ...
    def evaluate_retrieval(
        ...
        if ground_truth:
            for k in k_values:
                basic_metrics[f"recall@{k}"] = self.calculate_recall_at_k(
                    retrieved_ids, ground_truth, k
                )
                basic_metrics[f"precision@{k}"] = self.calculate_precision_at_k(
                    retrieved_ids, ground_truth, k
                )
            ...
            relevance_scores = [
                1.0 if doc_id in ground_truth else 0.0 for doc_id in retrieved_ids
            ]
            for k in k_values:
                basic_metrics[f"ndcg@{k}"] = self.calculate_ndcg_at_k(
                    retrieved_ids, relevance_scores, k
                )

        auto_metrics = self.evaluate_retrieval_with_auto_relevance(
            query, retrieved_docs, k_values
        )
    ...
    def calculate_distance_based_relevance(
        self, distance: float, max_distance: float = 0.4
    ) -> float:
    ...
    def determine_automatic_ground_truth(
        self, query: str, retrieved_docs: List[Dict], relevance_threshold: float = 0.6
    ) -> List[str]:
        ground_truth = []
    ...
    def evaluate_retrieval_with_auto_relevance(
        self,
        query: str,
        retrieved_docs: List[Dict],
        k_values: List[int] = None,
        ...
        auto_ground_truth = self.determine_automatic_ground_truth(
            query, retrieved_docs, relevance_threshold
        )

        metrics["auto_relevant_count"] = len(auto_ground_truth)
        metrics["auto_relevance_rate"] = (
            len(auto_ground_truth) / len(retrieved_docs) if retrieved_docs else 0.0
        )

        if auto_ground_truth:
            for k in k_values:
                metrics[f"auto_recall@{k}"] = self.calculate_recall_at_k(
                    retrieved_ids, auto_ground_truth, k
                )
                metrics[f"auto_precision@{k}"] = self.calculate_precision_at_k(
                    retrieved_ids, auto_ground_truth, k
                )
            ...
            for k in k_values:
                metrics[f"auto_ndcg@{k}"] = self.calculate_ndcg_at_k(
                    retrieved_ids, auto_relevance_scores, k
                )
    ...
    def evaluate_generation(
        self, query: str, response: str, context: str = "", reference: str = ""
    ) -> Dict[str, Any]:
        metrics = {
        ...
        if reference:
            metrics["reference_similarity"] = self._calculate_simple_similarity(
                response, reference
            )
    ...
    def get_session_summary(self) -> Dict[str, Any]:
        ...
        avg_retrieved_docs = np.mean(
            [m["retrieved_docs_count"] for m in self.session_metrics]
        )
        ...
        return {
            ...
            "total_session_time": (
                time.time() - self.session_metrics[0]["timestamp"]
