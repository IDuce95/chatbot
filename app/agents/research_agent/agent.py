...
class ResearchAgent(BaseAgent):
    ...
    def conduct_research(self, state: AgentState) -> AgentState:
        ...
        try:
            improvement_feedback = state.get("metadata", {}).get(
            ...
            if iteration_count > 0 and improvement_feedback:
                ...
                enhanced_query = self._enhance_query_with_feedback(
                    user_query, improvement_feedback, quality_issues
                )
            ...
            if self.rag_manager:
                ...
                context, has_relevant_docs, num_sources, source_files = (
                    self.rag_manager.get_context_with_relevance(refined_query)
                )

                if has_relevant_docs and context:
                    print(
                        f"ResearchAgent: Found relevant context from {num_sources} sources"
                    )
                    ...
                    print(
                        "❌ ResearchAgent: No relevant documents found in vector database"
                    )
    ...
    def _enhance_query_with_feedback(
        self, original_query: str, feedback: str, issues: list
    ) -> str:
        enhancements = []

        if "completeness" in feedback.lower() or any(
            "completeness" in issue for issue in issues
        ):
            enhancements.append("comprehensive detailed")
        if "accuracy" in feedback.lower() or any(
            "accuracy" in issue for issue in issues
        ):
        ...
        if "practical" in feedback.lower() or any(
            "practical" in issue for issue in issues
        ):
    ...
    def _refine_query(self, query: str) -> str:
        refinement_prompt = self.config["agents"]["research"][
            "query_refinement"
        ].format(query=query)
